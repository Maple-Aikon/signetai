...
\`)}\`}var vH=$9();var hH=I6(r6(),1);function U8(X){return typeof X==="object"&&X!==null}function LX(X){let Z=Reflect.get(globalThis,"process");if(!U8(Z))return;let $=Reflect.get(Z,"env");if(!U8($))return;let H=Reflect.get($,X);return typeof H==="string"?H:void 0}function c(X){if(typeof X!=="string")return;let Z=X.trim();return Z.length>0?Z:void 0}function d8(X){return c(LX(X))}import{existsSync as z9,readFileSync as Q9}from"node:fs";function QZ(X){return X.replace(/\\s*\\r?\\n\\s*/g," ").trim()}function W9(X){switch(X){case"assistant":return"Assistant";case"system":return"System";case"custom":return"Custom";case"tool":case"toolResult":case"bashExecution":case"pythonExecution":return"Tool";default:return"User"}}function H9(X){if(typeof X==="string"){let $=QZ(X);return $.length>0?$:void 0}if(!Array.isArray(X))return;let Z=[];for(let $ of X){if(!U8($))continue;let H=c($.text)??c($.input_text)??c($.content);if(!H)continue;Z.push(QZ(H))}if(Z.length===0)return;return Z.join(" ")}function WZ(X,Z){let $=H9(Z);if(!$)return;return\`\${W9(X)}: \${$}\`}function J9(X,Z){if(!U8(X)||typeof X.type!=="string")return;if(X.type==="custom_message"){if(typeof X.customType==="string"&&Z.has(X.customType))return;return WZ("custom",X.content)}if(X.type!=="message")return;if(!U8(X.message))return;let $=c(X.message.role),H=Reflect.get(X.message,"content")??Reflect.get(X.message,"parts");return WZ($,H)}function sX(X,Z=new Set){let $=[];for(let H of X){let U=J9(H,Z);if(!U)continue;$.push(U)}if($.length===0)return;return $.join(\`
\`)}function Y9(X){try{return JSON.parse(X)}catch{return}}function V9(X){let Z,$=[];for(let H of X){let U=Y9(H);if(!U8(U)||typeof U.type!=="string")continue;if(U.type==="session"){Z=U;continue}$.push(U)}return{header:Z,entries:$}}function _9(X){return Q9(X,"utf-8").split(\`
\`).map((Z)=>Z.trim()).filter((Z)=>Z.length>0)}function q9(X){return c(X?.cwd)??c(X?.project)??c(X?.workspace)}function rX(X,Z=new Set){if(!X||!z9(X))return{loaded:!1,sessionId:void 0,project:void 0,transcript:void 0};try{let{header:$,entries:H}=V9(_9(X));return{loaded:!0,sessionId:c($?.id),project:q9($),transcript:sX(H,Z)}}catch{return{loaded:!1,sessionId:void 0,project:void 0,transcript:void 0}}}function B9(X){return{"Content-Type":"application/json","x-signet-runtime-path":X.runtimePath,"x-signet-actor":X.actorName,"x-signet-actor-type":"harness"}}function U9(X){return X instanceof DOMException&&X.name==="TimeoutError"}async function HZ(X,Z,$,H={}){let{method:U="POST",body:G,timeout:_=$.defaultTimeout}=H;try{let q={method:U,headers:B9($),signal:AbortSignal.timeout(_)};if(G!==void 0)q.body=JSON.stringify(G);let B=await fetch(\`\${X}\${Z}\`,q);if(!B.ok)return console.warn(\`[\${$.logPrefix}] \${U} \${Z} failed: \${B.status}\`),{ok:!1,reason:"http",status:B.status};try{return{ok:!0,data:await B.json()}}catch{return console.warn(\`[\${$.logPrefix}] \${U} \${Z} returned invalid JSON\`),{ok:!1,reason:"invalid-json",status:B.status}}}catch(q){if(U9(q))return console.warn(\`[\${$.logPrefix}] \${U} \${Z} timed out after \${_}ms\`),{ok:!1,reason:"timeout"};return console.warn(\`[\${$.logPrefix}] \${U} \${Z} error:\`,q),{ok:!1,reason:"offline"}}}function u2(X,Z){return{async post($,H,U=Z.defaultTimeout){let G=await HZ(X,$,Z,{method:"POST",body:H,timeout:U});if(!G.ok)return null;return G.data},postResult($,H,U=Z.defaultTimeout){return HZ(X,$,Z,{method:"POST",body:H,timeout:U})}}}import{homedir as G9}from"node:os";import{join as M9}from"node:path";function m2(X){let Z=d8("SIGNET_PATH")??M9(G9(),".agents");return zZ(Z)??""}function A9(X){let Z=X.sessionManager.getBranch();if(Array.isArray(Z)&&Z.length>0)return Z;let $=X.sessionManager.getEntries();return Array.isArray($)?$:[]}function I0(X){let Z=X.sessionManager.getHeader(),$=c(X.sessionManager.getSessionId())??c(Z?.id),H=c(X.sessionManager.getSessionFile()),U=c(X.cwd)??c(Z?.cwd)??c(Z?.project)??c(Z?.workspace);return{sessionId:$,sessionFile:H,project:U}}async function OX(X,Z){return await X.client.post("/api/hooks/session-end",{harness:X.config.harness,runtimePath:X.config.runtimePath,reason:Z.reason,sessionKey:Z.sessionId,sessionId:Z.sessionId,agentId:Z.agentId,cwd:Z.project,...Z.transcript?{transcript:Z.transcript}:{}},X.config.writeTimeout)!==null}async function c8(X){for(let Z of X.state.getPendingSessionEnds()){if(X.state.sessionAlreadyEnded(Z.sessionId)){X.state.clearPendingSessionEnd(Z.sessionId);continue}let $=rX(Z.sessionFile,X.config.excludedCustomTypes);if(!$.loaded){await OX(X,{sessionId:Z.sessionId,agentId:Z.agentId,transcript:void 0,reason:Z.reason,project:void 0});continue}if(!await OX(X,{sessionId:$.sessionId??Z.sessionId,agentId:Z.agentId,transcript:$.transcript,reason:Z.reason,project:$.project}))continue;X.state.markSessionEnded(Z.sessionId),X.state.clearPendingSessionData(Z.sessionId),X.state.clearPendingSessionEnd(Z.sessionId)}}async function G8(X,Z){await c8(X);let $=I0(Z);X.state.setActiveSession($.sessionId,$.sessionFile),X.state.clearSessionEnded($.sessionId);let H=await X.client.postResult("/api/hooks/session-start",{harness:X.config.harness,project:$.project,agentId:X.agentId,sessionKey:$.sessionId,runtimePath:X.config.runtimePath},X.config.sessionStartTimeout()),U=H.ok?H.data.inject??H.data.recentContext??"":X.config.staticFallback(H.reason==="timeout"?"timeout":"offline");X.state.setSessionContext(U),X.state.setPendingSessionContext($.sessionId,U)}async function p8(X,Z){await c8(X);let $=I0(Z);if(!$.sessionId)return;if($.sessionId===X.state.getActiveSessionId()&&$.sessionFile===X.state.getActiveSessionFile())return;await G8(X,Z)}async function tX(X,Z,$){await c8(X);let H=I0(Z);if(X.state.sessionAlreadyEnded(H.sessionId))return;if(!await OX(X,{sessionId:H.sessionId,agentId:X.agentId,transcript:sX(A9(Z),X.config.excludedCustomTypes),reason:$,project:H.project}))return;X.state.markSessionEnded(H.sessionId),X.state.clearPendingSessionData(H.sessionId)}async function FX(X,Z,$){let H=c(Z.previousSessionFile)??X.state.getActiveSessionFile(),U=rX(H,X.config.excludedCustomTypes),G=U.sessionId??X.state.getActiveSessionId();if(X.state.sessionAlreadyEnded(G))return;if(!U.loaded){if(G)await OX(X,{sessionId:G,agentId:X.agentId,transcript:void 0,reason:$,project:void 0});if(G&&H)X.state.queuePendingSessionEnd(G,H,X.agentId,$);return}if(!await OX(X,{sessionId:G,agentId:X.agentId,transcript:U.transcript,reason:$,project:U.project})){if(G&&H)X.state.queuePendingSessionEnd(G,H,X.agentId,$);return}X.state.markSessionEnded(G),X.state.clearPendingSessionData(G)}async function NX(X,Z,$){await c8(X);let H=c($);if(!H)return;await p8(X,Z);let U=I0(Z);if(!U.sessionId)return;let G=await X.client.post("/api/hooks/user-prompt-submit",{harness:X.config.harness,project:U.project,agentId:X.agentId,sessionKey:U.sessionId,userMessage:H,runtimePath:X.config.runtimePath},X.config.promptSubmitTimeout);if(!G)return;if(G.sessionKnown===!1)await G8(X,Z);let _=c(G.inject);if(_)X.state.queuePendingRecall(U.sessionId,_)}var d2=64,JZ=4,YZ=128;function c2(X){return X.replace(/<\\/signet-memory>/gi,"<\\\\/signet-memory>")}function eX(X,Z){if(X.size<Z)return;let $=X.keys().next().value;if($!==void 0)X.delete($)}class X2{pendingSessionContext=new Map;pendingRecall=new Map;pendingSessionEnds=new Map;endedSessions=new Map;activeSessionId;activeSessionFile;sessionContext="";setActiveSession(X,Z){this.activeSessionId=X,this.activeSessionFile=Z}getActiveSessionId(){return this.activeSessionId}getActiveSessionFile(){return this.activeSessionFile}setSessionContext(X){this.sessionContext=X}getSessionContext(){return this.sessionContext}markSessionEnded(X){if(!X)return;if(!this.endedSessions.has(X))eX(this.endedSessions,YZ);this.endedSessions.set(X,Date.now())}clearSessionEnded(X){if(!X)return;this.endedSessions.delete(X)}sessionAlreadyEnded(X){if(!X)return!1;return this.endedSessions.has(X)}setPendingSessionContext(X,Z){if(!X)return;let $=c(Z);if($){this.pendingSessionContext.set(X,$);return}this.pendingSessionContext.delete(X)}clearPendingSessionContext(X){if(!X)return;this.pendingSessionContext.delete(X)}queuePendingRecall(X,Z){if(!this.pendingRecall.has(X))eX(this.pendingRecall,d2);let $=this.pendingRecall.get(X)??[];$.push(Z);while($.length>JZ)$.shift();this.pendingRecall.set(X,$)}clearPendingRecall(X){if(!X)return;this.pendingRecall.delete(X)}clearPendingSessionData(X){if(!X)return;this.pendingSessionContext.delete(X),this.pendingRecall.delete(X),this.pendingSessionEnds.delete(X)}hasPendingRecall(X){if(!X)return!1;let Z=this.pendingRecall.get(X);return Array.isArray(Z)&&Z.length>0}consumePendingRecall(X){if(!X)return;let Z=this.pendingRecall.get(X);if(!Z||Z.length===0)return;let $=Z.shift();if(Z.length===0)this.pendingRecall.delete(X);return c($)}queuePendingSessionEnd(X,Z,$,H){if(!this.pendingSessionEnds.has(X))eX(this.pendingSessionEnds,d2);this.pendingSessionEnds.set(X,{sessionId:X,sessionFile:Z,agentId:$,reason:H})}clearPendingSessionEnd(X){if(!X)return;this.pendingSessionEnds.delete(X)}getPendingSessionEnds(){return Array.from(this.pendingSessionEnds.values())}}var D0={};C2(D0,{IsUndefined:()=>H0,IsUint8Array:()=>H8,IsSymbol:()=>o2,IsString:()=>a,IsRegExp:()=>KX,IsObject:()=>l,IsNumber:()=>T0,IsNull:()=>i2,IsIterator:()=>n2,IsFunction:()=>l2,IsDate:()=>I8,IsBoolean:()=>W8,IsBigInt:()=>RX,IsAsyncIterator:()=>p2,IsArray:()=>W0,HasPropertyKey:()=>Z2});function Z2(X,Z){return Z in X}function p2(X){return l(X)&&!W0(X)&&!H8(X)&&Symbol.asyncIterator in X}function W0(X){return Array.isArray(X)}function RX(X){return typeof X==="bigint"}function W8(X){return typeof X==="boolean"}function I8(X){return X instanceof globalThis.Date}function l2(X){return typeof X==="function"}function n2(X){return l(X)&&!W0(X)&&!H8(X)&&Symbol.iterator in X}function i2(X){return X===null}function T0(X){return typeof X==="number"}function l(X){return typeof X==="object"&&X!==null}function KX(X){return X instanceof globalThis.RegExp}function a(X){return typeof X==="string"}function o2(X){return typeof X==="symbol"}function H8(X){return X instanceof globalThis.Uint8Array}function H0(X){return X===void 0}function D9(X){return X.map((Z)=>$2(Z))}function L9(X){return new Date(X.getTime())}function O9(X){return new Uint8Array(X)}function F9(X){return new RegExp(X.source,X.flags)}function N9(X){let Z={};for(let $ of Object.getOwnPropertyNames(X))Z[$]=$2(X[$]);for(let $ of Object.getOwnPropertySymbols(X))Z[$]=$2(X[$]);return Z}function $2(X){return W0(X)?D9(X):I8(X)?L9(X):H8(X)?O9(X):KX(X)?F9(X):l(X)?N9(X):X}function t(X){return $2(X)}function l8(X,Z){return Z===void 0?t(X):t({...Z,...X})}function VZ(X){return X!==null&&typeof X==="object"}function _Z(X){return globalThis.Array.isArray(X)&&!globalThis.ArrayBuffer.isView(X)}function qZ(X){return X===void 0}function BZ(X){return typeof X==="number"}var z2;(function(X){X.InstanceMode="default",X.ExactOptionalPropertyTypes=!1,X.AllowArrayObject=!1,X.AllowNaN=!1,X.AllowNullVoid=!1;function Z(_,q){return X.ExactOptionalPropertyTypes?q in _:_[q]!==void 0}X.IsExactOptionalProperty=Z;function $(_){let q=VZ(_);return X.AllowArrayObject?q:q&&!_Z(_)}X.IsObjectLike=$;function H(_){return $(_)&&!(_ instanceof Date)&&!(_ instanceof Uint8Array)}X.IsRecordLike=H;function U(_){return X.AllowNaN?BZ(_):Number.isFinite(_)}X.IsNumberLike=U;function G(_){let q=qZ(_);return X.AllowNullVoid?q||_===null:q}X.IsVoidLike=G})(z2||(z2={}));function R9(X){return globalThis.Object.freeze(X).map((Z)=>wX(Z))}function K9(X){return X}function w9(X){return X}function S9(X){return X}function j9(X){let Z={};for(let $ of Object.getOwnPropertyNames(X))Z[$]=wX(X[$]);for(let $ of Object.getOwnPropertySymbols(X))Z[$]=wX(X[$]);return globalThis.Object.freeze(Z)}function wX(X){return W0(X)?R9(X):I8(X)?K9(X):H8(X)?w9(X):KX(X)?S9(X):l(X)?j9(X):X}function w(X,Z){let $=Z!==void 0?{...Z,...X}:X;switch(z2.InstanceMode){case"freeze":return wX($);case"clone":return t($);default:return $}}class M0 extends Error{constructor(X){super(X)}}var Y0=Symbol.for("TypeBox.Transform"),l0=Symbol.for("TypeBox.Readonly"),A0=Symbol.for("TypeBox.Optional"),v0=Symbol.for("TypeBox.Hint"),C=Symbol.for("TypeBox.Kind");function n8(X){return l(X)&&X[l0]==="Readonly"}function S0(X){return l(X)&&X[A0]==="Optional"}function a2(X){return g(X,"Any")}function s2(X){return g(X,"Argument")}function n0(X){return g(X,"Array")}function T8(X){return g(X,"AsyncIterator")}function P8(X){return g(X,"BigInt")}function J8(X){return g(X,"Boolean")}function i0(X){return g(X,"Computed")}function o0(X){return g(X,"Constructor")}function C9(X){return g(X,"Date")}function a0(X){return g(X,"Function")}function s0(X){return g(X,"Integer")}function Z0(X){return g(X,"Intersect")}function k8(X){return g(X,"Iterator")}function g(X,Z){return l(X)&&C in X&&X[C]===Z}function Q2(X){return W8(X)||T0(X)||a(X)}function P0(X){return g(X,"Literal")}function k0(X){return g(X,"MappedKey")}function e(X){return g(X,"MappedResult")}function M8(X){return g(X,"Never")}function E9(X){return g(X,"Not")}function SX(X){return g(X,"Null")}function r0(X){return g(X,"Number")}function J0(X){return g(X,"Object")}function b8(X){return g(X,"Promise")}function f8(X){return g(X,"Record")}function Q0(X){return g(X,"Ref")}function r2(X){return g(X,"RegExp")}function Y8(X){return g(X,"String")}function jX(X){return g(X,"Symbol")}function b0(X){return g(X,"TemplateLiteral")}function I9(X){return g(X,"This")}function A8(X){return l(X)&&Y0 in X}function f0(X){return g(X,"Tuple")}function CX(X){return g(X,"Undefined")}function v(X){return g(X,"Union")}function T9(X){return g(X,"Uint8Array")}function P9(X){return g(X,"Unknown")}function k9(X){return g(X,"Unsafe")}function b9(X){return g(X,"Void")}function f9(X){return l(X)&&C in X&&a(X[C])}function x0(X){return a2(X)||s2(X)||n0(X)||J8(X)||P8(X)||T8(X)||i0(X)||o0(X)||C9(X)||a0(X)||s0(X)||Z0(X)||k8(X)||P0(X)||k0(X)||e(X)||M8(X)||E9(X)||SX(X)||r0(X)||J0(X)||b8(X)||f8(X)||Q0(X)||r2(X)||Y8(X)||jX(X)||b0(X)||I9(X)||f0(X)||CX(X)||v(X)||T9(X)||P9(X)||k9(X)||b9(X)||f9(X)}var O={};C2(O,{TypeGuardUnknownTypeError:()=>UZ,IsVoid:()=>X5,IsUnsafe:()=>eZ,IsUnknown:()=>tZ,IsUnionLiteral:()=>c9,IsUnion:()=>X6,IsUndefined:()=>sZ,IsUint8Array:()=>rZ,IsTuple:()=>aZ,IsTransform:()=>oZ,IsThis:()=>iZ,IsTemplateLiteral:()=>nZ,IsSymbol:()=>lZ,IsString:()=>pZ,IsSchema:()=>G0,IsRegExp:()=>cZ,IsRef:()=>dZ,IsRecursive:()=>d9,IsRecord:()=>mZ,IsReadonly:()=>h9,IsProperties:()=>W2,IsPromise:()=>uZ,IsOptional:()=>y9,IsObject:()=>yZ,IsNumber:()=>hZ,IsNull:()=>vZ,IsNot:()=>gZ,IsNever:()=>xZ,IsMappedResult:()=>fZ,IsMappedKey:()=>bZ,IsLiteralValue:()=>kZ,IsLiteralString:()=>TZ,IsLiteralNumber:()=>PZ,IsLiteralBoolean:()=>m9,IsLiteral:()=>IX,IsKindOf:()=>x,IsKind:()=>Z5,IsIterator:()=>IZ,IsIntersect:()=>EZ,IsInteger:()=>CZ,IsImport:()=>u9,IsFunction:()=>jZ,IsDate:()=>SZ,IsConstructor:()=>wZ,IsComputed:()=>KZ,IsBoolean:()=>RZ,IsBigInt:()=>NZ,IsAsyncIterator:()=>FZ,IsArray:()=>OZ,IsArgument:()=>LZ,IsAny:()=>DZ});class UZ extends M0{}var x9=["Argument","Any","Array","AsyncIterator","BigInt","Boolean","Computed","Constructor","Date","Enum","Function","Integer","Intersect","Iterator","Literal","MappedKey","MappedResult","Not","Null","Number","Object","Promise","Record","Ref","RegExp","String","Symbol","TemplateLiteral","This","Tuple","Undefined","Union","Uint8Array","Unknown","Void"];function GZ(X){try{return new RegExp(X),!0}catch{return!1}}function t2(X){if(!a(X))return!1;for(let Z=0;Z<X.length;Z++){let $=X.charCodeAt(Z);if($>=7&&$<=13||$===27||$===127)return!1}return!0}function MZ(X){return e2(X)||G0(X)}function EX(X){return H0(X)||RX(X)}function s(X){return H0(X)||T0(X)}function e2(X){return H0(X)||W8(X)}function n(X){return H0(X)||a(X)}function g9(X){return H0(X)||a(X)&&t2(X)&&GZ(X)}function v9(X){return H0(X)||a(X)&&t2(X)}function AZ(X){return H0(X)||G0(X)}function h9(X){return l(X)&&X[l0]==="Readonly"}function y9(X){return l(X)&&X[A0]==="Optional"}function DZ(X){return x(X,"Any")&&n(X.$id)}function LZ(X){return x(X,"Argument")&&T0(X.index)}function OZ(X){return x(X,"Array")&&X.type==="array"&&n(X.$id)&&G0(X.items)&&s(X.minItems)&&s(X.maxItems)&&e2(X.uniqueItems)&&AZ(X.contains)&&s(X.minContains)&&s(X.maxContains)}function FZ(X){return x(X,"AsyncIterator")&&X.type==="AsyncIterator"&&n(X.$id)&&G0(X.items)}function NZ(X){return x(X,"BigInt")&&X.type==="bigint"&&n(X.$id)&&EX(X.exclusiveMaximum)&&EX(X.exclusiveMinimum)&&EX(X.maximum)&&EX(X.minimum)&&EX(X.multipleOf)}function RZ(X){return x(X,"Boolean")&&X.type==="boolean"&&n(X.$id)}function KZ(X){return x(X,"Computed")&&a(X.target)&&W0(X.parameters)&&X.parameters.every((Z)=>G0(Z))}function wZ(X){return x(X,"Constructor")&&X.type==="Constructor"&&n(X.$id)&&W0(X.parameters)&&X.parameters.every((Z)=>G0(Z))&&G0(X.returns)}function SZ(X){return x(X,"Date")&&X.type==="Date"&&n(X.$id)&&s(X.exclusiveMaximumTimestamp)&&s(X.exclusiveMinimumTimestamp)&&s(X.maximumTimestamp)&&s(X.minimumTimestamp)&&s(X.multipleOfTimestamp)}function jZ(X){return x(X,"Function")&&X.type==="Function"&&n(X.$id)&&W0(X.parameters)&&X.parameters.every((Z)=>G0(Z))&&G0(X.returns)}function u9(X){return x(X,"Import")&&Z2(X,"$defs")&&l(X.$defs)&&W2(X.$defs)&&Z2(X,"$ref")&&a(X.$ref)&&X.$ref in X.$defs}function CZ(X){return x(X,"Integer")&&X.type==="integer"&&n(X.$id)&&s(X.exclusiveMaximum)&&s(X.exclusiveMinimum)&&s(X.maximum)&&s(X.minimum)&&s(X.multipleOf)}function W2(X){return l(X)&&Object.entries(X).every(([Z,$])=>t2(Z)&&G0($))}function EZ(X){return x(X,"Intersect")&&(a(X.type)&&X.type!=="object"?!1:!0)&&W0(X.allOf)&&X.allOf.every((Z)=>G0(Z)&&!oZ(Z))&&n(X.type)&&(e2(X.unevaluatedProperties)||AZ(X.unevaluatedProperties))&&n(X.$id)}function IZ(X){return x(X,"Iterator")&&X.type==="Iterator"&&n(X.$id)&&G0(X.items)}function x(X,Z){return l(X)&&C in X&&X[C]===Z}function TZ(X){return IX(X)&&a(X.const)}function PZ(X){return IX(X)&&T0(X.const)}function m9(X){return IX(X)&&W8(X.const)}function IX(X){return x(X,"Literal")&&n(X.$id)&&kZ(X.const)}function kZ(X){return W8(X)||T0(X)||a(X)}function bZ(X){return x(X,"MappedKey")&&W0(X.keys)&&X.keys.every((Z)=>T0(Z)||a(Z))}function fZ(X){return x(X,"MappedResult")&&W2(X.properties)}function xZ(X){return x(X,"Never")&&l(X.not)&&Object.getOwnPropertyNames(X.not).length===0}function gZ(X){return x(X,"Not")&&G0(X.not)}function vZ(X){return x(X,"Null")&&X.type==="null"&&n(X.$id)}function hZ(X){return x(X,"Number")&&X.type==="number"&&n(X.$id)&&s(X.exclusiveMaximum)&&s(X.exclusiveMinimum)&&s(X.maximum)&&s(X.minimum)&&s(X.multipleOf)}function yZ(X){return x(X,"Object")&&X.type==="object"&&n(X.$id)&&W2(X.properties)&&MZ(X.additionalProperties)&&s(X.minProperties)&&s(X.maxProperties)}function uZ(X){return x(X,"Promise")&&X.type==="Promise"&&n(X.$id)&&G0(X.item)}function mZ(X){return x(X,"Record")&&X.type==="object"&&n(X.$id)&&MZ(X.additionalProperties)&&l(X.patternProperties)&&((Z)=>{let $=Object.getOwnPropertyNames(Z.patternProperties);return $.length===1&&GZ($[0])&&l(Z.patternProperties)&&G0(Z.patternProperties[$[0]])})(X)}function d9(X){return l(X)&&v0 in X&&X[v0]==="Recursive"}function dZ(X){return x(X,"Ref")&&n(X.$id)&&a(X.$ref)}function cZ(X){return x(X,"RegExp")&&n(X.$id)&&a(X.source)&&a(X.flags)&&s(X.maxLength)&&s(X.minLength)}function pZ(X){return x(X,"String")&&X.type==="string"&&n(X.$id)&&s(X.minLength)&&s(X.maxLength)&&g9(X.pattern)&&v9(X.format)}function lZ(X){return x(X,"Symbol")&&X.type==="symbol"&&n(X.$id)}function nZ(X){return x(X,"TemplateLiteral")&&X.type==="string"&&a(X.pattern)&&X.pattern[0]==="^"&&X.pattern[X.pattern.length-1]==="$"}function iZ(X){return x(X,"This")&&n(X.$id)&&a(X.$ref)}function oZ(X){return l(X)&&Y0 in X}function aZ(X){return x(X,"Tuple")&&X.type==="array"&&n(X.$id)&&T0(X.minItems)&&T0(X.maxItems)&&X.minItems===X.maxItems&&(H0(X.items)&&H0(X.additionalItems)&&X.minItems===0||W0(X.items)&&X.items.every((Z)=>G0(Z)))}function sZ(X){return x(X,"Undefined")&&X.type==="undefined"&&n(X.$id)}function c9(X){return X6(X)&&X.anyOf.every((Z)=>TZ(Z)||PZ(Z))}function X6(X){return x(X,"Union")&&n(X.$id)&&l(X)&&W0(X.anyOf)&&X.anyOf.every((Z)=>G0(Z))}function rZ(X){return x(X,"Uint8Array")&&X.type==="Uint8Array"&&n(X.$id)&&s(X.minByteLength)&&s(X.maxByteLength)}function tZ(X){return x(X,"Unknown")&&n(X.$id)}function eZ(X){return x(X,"Unsafe")}function X5(X){return x(X,"Void")&&X.type==="void"&&n(X.$id)}function Z5(X){return l(X)&&C in X&&a(X[C])&&!x9.includes(X[C])}function G0(X){return l(X)&&(DZ(X)||LZ(X)||OZ(X)||RZ(X)||NZ(X)||FZ(X)||KZ(X)||wZ(X)||SZ(X)||jZ(X)||CZ(X)||EZ(X)||IZ(X)||IX(X)||bZ(X)||fZ(X)||xZ(X)||gZ(X)||vZ(X)||hZ(X)||yZ(X)||uZ(X)||mZ(X)||dZ(X)||cZ(X)||pZ(X)||lZ(X)||nZ(X)||iZ(X)||aZ(X)||sZ(X)||X6(X)||rZ(X)||tZ(X)||eZ(X)||X5(X)||Z5(X))}var $5="(true|false)",H2="(0|[1-9][0-9]*)",z5="(.*)";var D8="^(0|[1-9][0-9]*)$",L8="^(.*)$",Q5="^(?!.*)$";function W5(X,Z){return X.includes(Z)}function H5(X){return[...new Set(X)]}function p9(X,Z){return X.filter(($)=>Z.includes($))}function l9(X,Z){return X.reduce(($,H)=>{return p9($,H)},Z)}function J5(X){return X.length===1?X[0]:X.length>1?l9(X.slice(1),X[0]):[]}function Y5(X){let Z=[];for(let $ of X)Z.push(...$);return Z}function O8(X){return w({[C]:"Any"},X)}function i8(X,Z){return w({[C]:"Array",type:"array",items:X},Z)}function V5(X){return w({[C]:"Argument",index:X})}function o8(X,Z){return w({[C]:"AsyncIterator",type:"AsyncIterator",items:X},Z)}function r(X,Z,$){return w({[C]:"Computed",target:X,parameters:Z},$)}function n9(X,Z){let{[Z]:$,...H}=X;return H}function $0(X,Z){return Z.reduce(($,H)=>n9($,H),X)}function h(X){return w({[C]:"Never",not:{}},X)}function y(X){return w({[C]:"MappedResult",properties:X})}function a8(X,Z,$){return w({[C]:"Constructor",type:"Constructor",parameters:X,returns:Z},$)}function Z8(X,Z,$){return w({[C]:"Function",type:"Function",parameters:X,returns:Z},$)}function TX(X,Z){return w({[C]:"Union",anyOf:X},Z)}function i9(X){return X.some((Z)=>S0(Z))}function _5(X){return X.map((Z)=>S0(Z)?o9(Z):Z)}function o9(X){return $0(X,[A0])}function a9(X,Z){return i9(X)?L0(TX(_5(X),Z)):TX(_5(X),Z)}function $8(X,Z){return X.length===1?w(X[0],Z):X.length===0?h(Z):a9(X,Z)}function d(X,Z){return X.length===0?h(Z):X.length===1?w(X[0],Z):TX(X,Z)}class Z6 extends M0{}function s9(X){return X.replace(/\\\\\\$/g,"$").replace(/\\\\\\*/g,"*").replace(/\\\\\\^/g,"^").replace(/\\\\\\|/g,"|").replace(/\\\\\\(/g,"(").replace(/\\\\\\)/g,")")}function $6(X,Z,$){return X[Z]===$&&X.charCodeAt(Z-1)!==92}function _8(X,Z){return $6(X,Z,"(")}function PX(X,Z){return $6(X,Z,")")}function q5(X,Z){return $6(X,Z,"|")}function r9(X){if(!(_8(X,0)&&PX(X,X.length-1)))return!1;let Z=0;for(let $=0;$<X.length;$++){if(_8(X,$))Z+=1;if(PX(X,$))Z-=1;if(Z===0&&$!==X.length-1)return!1}return!0}function t9(X){return X.slice(1,X.length-1)}function e9(X){let Z=0;for(let $=0;$<X.length;$++){if(_8(X,$))Z+=1;if(PX(X,$))Z-=1;if(q5(X,$)&&Z===0)return!0}return!1}function Xz(X){for(let Z=0;Z<X.length;Z++)if(_8(X,Z))return!0;return!1}function Zz(X){let[Z,$]=[0,0],H=[];for(let G=0;G<X.length;G++){if(_8(X,G))Z+=1;if(PX(X,G))Z-=1;if(q5(X,G)&&Z===0){let _=X.slice($,G);if(_.length>0)H.push(s8(_));$=G+1}}let U=X.slice($);if(U.length>0)H.push(s8(U));if(H.length===0)return{type:"const",const:""};if(H.length===1)return H[0];return{type:"or",expr:H}}function $z(X){function Z(U,G){if(!_8(U,G))throw new Z6("TemplateLiteralParser: Index must point to open parens");let _=0;for(let q=G;q<U.length;q++){if(_8(U,q))_+=1;if(PX(U,q))_-=1;if(_===0)return[G,q]}throw new Z6("TemplateLiteralParser: Unclosed group parens in expression")}function $(U,G){for(let _=G;_<U.length;_++)if(_8(U,_))return[G,_];return[G,U.length]}let H=[];for(let U=0;U<X.length;U++)if(_8(X,U)){let[G,_]=Z(X,U),q=X.slice(G,_+1);H.push(s8(q)),U=_}else{let[G,_]=$(X,U),q=X.slice(G,_);if(q.length>0)H.push(s8(q));U=_-1}return H.length===0?{type:"const",const:""}:H.length===1?H[0]:{type:"and",expr:H}}function s8(X){return r9(X)?s8(t9(X)):e9(X)?Zz(X):Xz(X)?$z(X):{type:"const",const:s9(X)}}function r8(X){return s8(X.slice(1,X.length-1))}class B5 extends M0{}function zz(X){return X.type==="or"&&X.expr.length===2&&X.expr[0].type==="const"&&X.expr[0].const==="0"&&X.expr[1].type==="const"&&X.expr[1].const==="[1-9][0-9]*"}function Qz(X){return X.type==="or"&&X.expr.length===2&&X.expr[0].type==="const"&&X.expr[0].const==="true"&&X.expr[1].type==="const"&&X.expr[1].const==="false"}function Wz(X){return X.type==="const"&&X.const===".*"}function x8(X){return zz(X)||Wz(X)?!1:Qz(X)?!0:X.type==="and"?X.expr.every((Z)=>x8(Z)):X.type==="or"?X.expr.every((Z)=>x8(Z)):X.type==="const"?!0:(()=>{throw new B5("Unknown expression type")})()}function U5(X){let Z=r8(X.pattern);return x8(Z)}class G5 extends M0{}function*M5(X){if(X.length===1)return yield*X[0];for(let Z of X[0])for(let $ of M5(X.slice(1)))yield\`\${Z}\${$}\`}function*Hz(X){return yield*M5(X.expr.map((Z)=>[...kX(Z)]))}function*Jz(X){for(let Z of X.expr)yield*kX(Z)}function*Yz(X){return yield X.const}function*kX(X){return X.type==="and"?yield*Hz(X):X.type==="or"?yield*Jz(X):X.type==="const"?yield*Yz(X):(()=>{throw new G5("Unknown expression")})()}function J2(X){let Z=r8(X.pattern);return x8(Z)?[...kX(Z)]:[]}function m(X,Z){return w({[C]:"Literal",const:X,type:typeof X},Z)}function Y2(X){return w({[C]:"Boolean",type:"boolean"},X)}function t8(X){return w({[C]:"BigInt",type:"bigint"},X)}function h0(X){return w({[C]:"Number",type:"number"},X)}function t0(X){return w({[C]:"String",type:"string"},X)}function*Vz(X){let Z=X.trim().replace(/"|'/g,"");return Z==="boolean"?yield Y2():Z==="number"?yield h0():Z==="bigint"?yield t8():Z==="string"?yield t0():yield(()=>{let $=Z.split("|").map((H)=>m(H.trim()));return $.length===0?h():$.length===1?$[0]:$8($)})()}function*_z(X){if(X[1]!=="{"){let Z=m("$"),$=z6(X.slice(1));return yield*[Z,...$]}for(let Z=2;Z<X.length;Z++)if(X[Z]==="}"){let $=Vz(X.slice(2,Z)),H=z6(X.slice(Z+1));return yield*[...$,...H]}yield m(X)}function*z6(X){for(let Z=0;Z<X.length;Z++)if(X[Z]==="$"){let $=m(X.slice(0,Z)),H=_z(X.slice(Z));return yield*[$,...H]}yield m(X)}function A5(X){return[...z6(X)]}class D5 extends M0{}function qz(X){return X.replace(/[.*+?^\${}()|[\\]\\\\]/g,"\\\\$&")}function L5(X,Z){return b0(X)?X.pattern.slice(1,X.pattern.length-1):v(X)?\`(\${X.anyOf.map(($)=>L5($,Z)).join("|")})\`:r0(X)?\`\${Z}\${H2}\`:s0(X)?\`\${Z}\${H2}\`:P8(X)?\`\${Z}\${H2}\`:Y8(X)?\`\${Z}\${z5}\`:P0(X)?\`\${Z}\${qz(X.const.toString())}\`:J8(X)?\`\${Z}\${$5}\`:(()=>{throw new D5(\`Unexpected Kind '\${X[C]}'\`)})()}function Q6(X){return\`^\${X.map((Z)=>L5(Z,"")).join("")}$\`}function g8(X){let $=J2(X).map((H)=>m(H));return $8($)}function V2(X,Z){let $=a(X)?Q6(A5(X)):Q6(X);return w({[C]:"TemplateLiteral",type:"string",pattern:$},Z)}function Bz(X){return J2(X).map(($)=>$.toString())}function Uz(X){let Z=[];for(let $ of X)Z.push(...j0($));return Z}function Gz(X){return[X.toString()]}function j0(X){return[...new Set(b0(X)?Bz(X):v(X)?Uz(X.anyOf):P0(X)?Gz(X.const):r0(X)?["[number]"]:s0(X)?["[number]"]:[])]}function Mz(X,Z,$){let H={};for(let U of Object.getOwnPropertyNames(Z))H[U]=F8(X,j0(Z[U]),$);return H}function Az(X,Z,$){return Mz(X,Z.properties,$)}function O5(X,Z,$){let H=Az(X,Z,$);return y(H)}function N5(X,Z){return X.map(($)=>R5($,Z))}function Dz(X){return X.filter((Z)=>!M8(Z))}function Lz(X,Z){return _2(Dz(N5(X,Z)))}function Oz(X){return X.some((Z)=>M8(Z))?[]:X}function Fz(X,Z){return $8(Oz(N5(X,Z)))}function Nz(X,Z){return Z in X?X[Z]:Z==="[number]"?$8(X):h()}function Rz(X,Z){return Z==="[number]"?X:h()}function Kz(X,Z){return Z in X?X[Z]:h()}function R5(X,Z){return Z0(X)?Lz(X.allOf,Z):v(X)?Fz(X.anyOf,Z):f0(X)?Nz(X.items??[],Z):n0(X)?Rz(X.items,Z):J0(X)?Kz(X.properties,Z):h()}function W6(X,Z){return Z.map(($)=>R5(X,$))}function F5(X,Z){return $8(W6(X,Z))}function F8(X,Z,$){if(Q0(X)||Q0(Z)){if(!x0(X)||!x0(Z))throw new M0("Index types using Ref parameters require both Type and Key to be of TSchema");return r("Index",[X,Z])}if(e(Z))return O5(X,Z,$);if(k0(Z))return K5(X,Z,$);return w(x0(Z)?F5(X,j0(Z)):F5(X,Z),$)}function wz(X,Z,$){return{[Z]:F8(X,[Z],t($))}}function Sz(X,Z,$){return Z.reduce((H,U)=>{return{...H,...wz(X,U,$)}},{})}function jz(X,Z,$){return Sz(X,Z.keys,$)}function K5(X,Z,$){let H=jz(X,Z,$);return y(H)}function e8(X,Z){return w({[C]:"Iterator",type:"Iterator",items:X},Z)}function Cz(X){return globalThis.Object.keys(X).filter((Z)=>!S0(X[Z]))}function Ez(X,Z){let $=Cz(X),H=$.length>0?{[C]:"Object",type:"object",required:$,properties:X}:{[C]:"Object",type:"object",properties:X};return w(H,Z)}var i=Ez;function q2(X,Z){return w({[C]:"Promise",type:"Promise",item:X},Z)}function Iz(X){return w($0(X,[l0]))}function Tz(X){return w({...X,[l0]:"Readonly"})}function Pz(X,Z){return Z===!1?Iz(X):Tz(X)}function C0(X,Z){let $=Z??!0;return e(X)?w5(X,$):Pz(X,$)}function kz(X,Z){let $={};for(let H of globalThis.Object.getOwnPropertyNames(X))$[H]=C0(X[H],Z);return $}function bz(X,Z){return kz(X.properties,Z)}function w5(X,Z){let $=bz(X,Z);return y($)}function y0(X,Z){return w(X.length>0?{[C]:"Tuple",type:"array",items:X,additionalItems:!1,minItems:X.length,maxItems:X.length}:{[C]:"Tuple",type:"array",minItems:X.length,maxItems:X.length},Z)}function S5(X,Z){return X in Z?u0(X,Z[X]):y(Z)}function fz(X){return{[X]:m(X)}}function xz(X){let Z={};for(let $ of X)Z[$]=m($);return Z}function gz(X,Z){return W5(Z,X)?fz(X):xz(Z)}function vz(X,Z){let $=gz(X,Z);return S5(X,$)}function bX(X,Z){return Z.map(($)=>u0(X,$))}function hz(X,Z){let $={};for(let H of globalThis.Object.getOwnPropertyNames(Z))$[H]=u0(X,Z[H]);return $}function u0(X,Z){let $={...Z};return S0(Z)?L0(u0(X,$0(Z,[A0]))):n8(Z)?C0(u0(X,$0(Z,[l0]))):e(Z)?S5(X,Z.properties):k0(Z)?vz(X,Z.keys):o0(Z)?a8(bX(X,Z.parameters),u0(X,Z.returns),$):a0(Z)?Z8(bX(X,Z.parameters),u0(X,Z.returns),$):T8(Z)?o8(u0(X,Z.items),$):k8(Z)?e8(u0(X,Z.items),$):Z0(Z)?O0(bX(X,Z.allOf),$):v(Z)?d(bX(X,Z.anyOf),$):f0(Z)?y0(bX(X,Z.items??[]),$):J0(Z)?i(hz(X,Z.properties),$):n0(Z)?i8(u0(X,Z.items),$):b8(Z)?q2(u0(X,Z.item),$):Z}function yz(X,Z){let $={};for(let H of X)$[H]=u0(H,Z);return $}function j5(X,Z,$){let H=x0(X)?j0(X):X,U=Z({[C]:"MappedKey",keys:H}),G=yz(H,U);return i(G,$)}function uz(X){return w($0(X,[A0]))}function mz(X){return w({...X,[A0]:"Optional"})}function dz(X,Z){return Z===!1?uz(X):mz(X)}function L0(X,Z){let $=Z??!0;return e(X)?C5(X,$):dz(X,$)}function cz(X,Z){let $={};for(let H of globalThis.Object.getOwnPropertyNames(X))$[H]=L0(X[H],Z);return $}function pz(X,Z){return cz(X.properties,Z)}function C5(X,Z){let $=pz(X,Z);return y($)}function fX(X,Z={}){let $=X.every((U)=>J0(U)),H=x0(Z.unevaluatedProperties)?{unevaluatedProperties:Z.unevaluatedProperties}:{};return w(Z.unevaluatedProperties===!1||x0(Z.unevaluatedProperties)||$?{...H,[C]:"Intersect",type:"object",allOf:X}:{...H,[C]:"Intersect",allOf:X},Z)}function lz(X){return X.every((Z)=>S0(Z))}function nz(X){return $0(X,[A0])}function E5(X){return X.map((Z)=>S0(Z)?nz(Z):Z)}function iz(X,Z){return lz(X)?L0(fX(E5(X),Z)):fX(E5(X),Z)}function _2(X,Z={}){if(X.length===1)return w(X[0],Z);if(X.length===0)return h(Z);if(X.some(($)=>A8($)))throw Error("Cannot intersect transform types");return iz(X,Z)}function O0(X,Z){if(X.length===1)return w(X[0],Z);if(X.length===0)return h(Z);if(X.some(($)=>A8($)))throw Error("Cannot intersect transform types");return fX(X,Z)}function z8(...X){let[Z,$]=typeof X[0]==="string"?[X[0],X[1]]:[X[0].$id,X[1]];if(typeof Z!=="string")throw new M0("Ref: $ref must be a string");return w({[C]:"Ref",$ref:Z},$)}function oz(X,Z){return r("Awaited",[r(X,Z)])}function az(X){return r("Awaited",[z8(X)])}function sz(X){return O0(I5(X))}function rz(X){return d(I5(X))}function tz(X){return XX(X)}function I5(X){return X.map((Z)=>XX(Z))}function XX(X,Z){return w(i0(X)?oz(X.target,X.parameters):Z0(X)?sz(X.allOf):v(X)?rz(X.anyOf):b8(X)?tz(X.item):Q0(X)?az(X.$ref):X,Z)}function T5(X){let Z=[];for(let $ of X)Z.push(xX($));return Z}function ez(X){let Z=T5(X);return Y5(Z)}function XQ(X){let Z=T5(X);return J5(Z)}function ZQ(X){return X.map((Z,$)=>$.toString())}function $Q(X){return["[number]"]}function zQ(X){return globalThis.Object.getOwnPropertyNames(X)}function QQ(X){if(!WQ)return[];return globalThis.Object.getOwnPropertyNames(X).map(($)=>{return $[0]==="^"&&$[$.length-1]==="$"?$.slice(1,$.length-1):$})}function xX(X){return Z0(X)?ez(X.allOf):v(X)?XQ(X.anyOf):f0(X)?ZQ(X.items??[]):n0(X)?$Q(X.items):J0(X)?zQ(X.properties):f8(X)?QQ(X.patternProperties):[]}var WQ=!1;function HQ(X,Z){return r("KeyOf",[r(X,Z)])}function JQ(X){return r("KeyOf",[z8(X)])}function YQ(X,Z){let $=xX(X),H=VQ($),U=$8(H);return w(U,Z)}function VQ(X){return X.map((Z)=>Z==="[number]"?h0():m(Z))}function ZX(X,Z){return i0(X)?HQ(X.target,X.parameters):Q0(X)?JQ(X.$ref):e(X)?P5(X,Z):YQ(X,Z)}function _Q(X,Z){let $={};for(let H of globalThis.Object.getOwnPropertyNames(X))$[H]=ZX(X[H],t(Z));return $}function qQ(X,Z){return _Q(X.properties,Z)}function P5(X,Z){let $=qQ(X,Z);return y($)}function BQ(X){let Z=[];for(let $ of X)Z.push(...xX($));return H5(Z)}function UQ(X){return X.filter((Z)=>!M8(Z))}function GQ(X,Z){let $=[];for(let H of X)$.push(...W6(H,[Z]));return UQ($)}function MQ(X,Z){let $={};for(let H of Z)$[H]=_2(GQ(X,H));return $}function k5(X,Z){let $=BQ(X),H=MQ(X,$);return i(H,Z)}function B2(X){return w({[C]:"Date",type:"Date"},X)}function U2(X){return w({[C]:"Null",type:"null"},X)}function G2(X){return w({[C]:"Symbol",type:"symbol"},X)}function M2(X){return w({[C]:"Undefined",type:"undefined"},X)}function A2(X){return w({[C]:"Uint8Array",type:"Uint8Array"},X)}function N8(X){return w({[C]:"Unknown"},X)}function AQ(X){return X.map((Z)=>H6(Z,!1))}function DQ(X){let Z={};for(let $ of globalThis.Object.getOwnPropertyNames(X))Z[$]=C0(H6(X[$],!1));return Z}function D2(X,Z){return Z===!0?X:C0(X)}function H6(X,Z){return p2(X)?D2(O8(),Z):n2(X)?D2(O8(),Z):W0(X)?C0(y0(AQ(X))):H8(X)?A2():I8(X)?B2():l(X)?D2(i(DQ(X)),Z):l2(X)?D2(Z8([],N8()),Z):H0(X)?M2():i2(X)?U2():o2(X)?G2():RX(X)?t8():T0(X)?m(X):W8(X)?m(X):a(X)?m(X):i({})}function b5(X,Z){return w(H6(X,!0),Z)}function f5(X,Z){return o0(X)?y0(X.parameters,Z):h(Z)}function x5(X,Z){if(H0(X))throw Error("Enum undefined or empty");let $=globalThis.Object.getOwnPropertyNames(X).filter((G)=>isNaN(G)).map((G)=>X[G]),U=[...new Set($)].map((G)=>m(G));return d(U,{...Z,[v0]:"Enum"})}class u5 extends M0{}var S;(function(X){X[X.Union=0]="Union",X[X.True=1]="True",X[X.False=2]="False"})(S||(S={}));function m0(X){return X===S.False?X:S.True}function $X(X){throw new u5(X)}function V0(X){return O.IsNever(X)||O.IsIntersect(X)||O.IsUnion(X)||O.IsUnknown(X)||O.IsAny(X)}function _0(X,Z){return O.IsNever(Z)?c5(X,Z):O.IsIntersect(Z)?L2(X,Z):O.IsUnion(Z)?q6(X,Z):O.IsUnknown(Z)?i5(X,Z):O.IsAny(Z)?_6(X,Z):$X("StructuralRight")}function _6(X,Z){return S.True}function LQ(X,Z){return O.IsIntersect(Z)?L2(X,Z):O.IsUnion(Z)&&Z.anyOf.some(($)=>O.IsAny($)||O.IsUnknown($))?S.True:O.IsUnion(Z)?S.Union:O.IsUnknown(Z)?S.True:O.IsAny(Z)?S.True:S.Union}function OQ(X,Z){return O.IsUnknown(X)?S.False:O.IsAny(X)?S.Union:O.IsNever(X)?S.True:S.False}function FQ(X,Z){return O.IsObject(Z)&&O2(Z)?S.True:V0(Z)?_0(X,Z):!O.IsArray(Z)?S.False:m0(o(X.items,Z.items))}function NQ(X,Z){return V0(Z)?_0(X,Z):!O.IsAsyncIterator(Z)?S.False:m0(o(X.items,Z.items))}function RQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):O.IsRecord(Z)?d0(X,Z):O.IsBigInt(Z)?S.True:S.False}function m5(X,Z){return O.IsLiteralBoolean(X)?S.True:O.IsBoolean(X)?S.True:S.False}function KQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):O.IsRecord(Z)?d0(X,Z):O.IsBoolean(Z)?S.True:S.False}function wQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):!O.IsConstructor(Z)?S.False:X.parameters.length>Z.parameters.length?S.False:!X.parameters.every(($,H)=>m0(o(Z.parameters[H],$))===S.True)?S.False:m0(o(X.returns,Z.returns))}function SQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):O.IsRecord(Z)?d0(X,Z):O.IsDate(Z)?S.True:S.False}function jQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):!O.IsFunction(Z)?S.False:X.parameters.length>Z.parameters.length?S.False:!X.parameters.every(($,H)=>m0(o(Z.parameters[H],$))===S.True)?S.False:m0(o(X.returns,Z.returns))}function d5(X,Z){return O.IsLiteral(X)&&D0.IsNumber(X.const)?S.True:O.IsNumber(X)||O.IsInteger(X)?S.True:S.False}function CQ(X,Z){return O.IsInteger(Z)||O.IsNumber(Z)?S.True:V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):O.IsRecord(Z)?d0(X,Z):S.False}function L2(X,Z){return Z.allOf.every(($)=>o(X,$)===S.True)?S.True:S.False}function EQ(X,Z){return X.allOf.some(($)=>o($,Z)===S.True)?S.True:S.False}function IQ(X,Z){return V0(Z)?_0(X,Z):!O.IsIterator(Z)?S.False:m0(o(X.items,Z.items))}function TQ(X,Z){return O.IsLiteral(Z)&&Z.const===X.const?S.True:V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):O.IsRecord(Z)?d0(X,Z):O.IsString(Z)?n5(X,Z):O.IsNumber(Z)?p5(X,Z):O.IsInteger(Z)?d5(X,Z):O.IsBoolean(Z)?m5(X,Z):S.False}function c5(X,Z){return S.False}function PQ(X,Z){return S.True}function g5(X){let[Z,$]=[X,0];while(!0){if(!O.IsNot(Z))break;Z=Z.not,$+=1}return $%2===0?Z:N8()}function kQ(X,Z){return O.IsNot(X)?o(g5(X),Z):O.IsNot(Z)?o(X,g5(Z)):$X("Invalid fallthrough for Not")}function bQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):O.IsRecord(Z)?d0(X,Z):O.IsNull(Z)?S.True:S.False}function p5(X,Z){return O.IsLiteralNumber(X)?S.True:O.IsNumber(X)||O.IsInteger(X)?S.True:S.False}function fQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):O.IsRecord(Z)?d0(X,Z):O.IsInteger(Z)||O.IsNumber(Z)?S.True:S.False}function E0(X,Z){return Object.getOwnPropertyNames(X.properties).length===Z}function v5(X){return O2(X)}function h5(X){return E0(X,0)||E0(X,1)&&"description"in X.properties&&O.IsUnion(X.properties.description)&&X.properties.description.anyOf.length===2&&(O.IsString(X.properties.description.anyOf[0])&&O.IsUndefined(X.properties.description.anyOf[1])||O.IsString(X.properties.description.anyOf[1])&&O.IsUndefined(X.properties.description.anyOf[0]))}function J6(X){return E0(X,0)}function y5(X){return E0(X,0)}function xQ(X){return E0(X,0)}function gQ(X){return E0(X,0)}function vQ(X){return O2(X)}function hQ(X){let Z=h0();return E0(X,0)||E0(X,1)&&"length"in X.properties&&m0(o(X.properties.length,Z))===S.True}function yQ(X){return E0(X,0)}function O2(X){let Z=h0();return E0(X,0)||E0(X,1)&&"length"in X.properties&&m0(o(X.properties.length,Z))===S.True}function uQ(X){let Z=Z8([O8()],O8());return E0(X,0)||E0(X,1)&&"then"in X.properties&&m0(o(X.properties.then,Z))===S.True}function l5(X,Z){return o(X,Z)===S.False?S.False:O.IsOptional(X)&&!O.IsOptional(Z)?S.False:S.True}function F0(X,Z){return O.IsUnknown(X)?S.False:O.IsAny(X)?S.Union:O.IsNever(X)||O.IsLiteralString(X)&&v5(Z)||O.IsLiteralNumber(X)&&J6(Z)||O.IsLiteralBoolean(X)&&y5(Z)||O.IsSymbol(X)&&h5(Z)||O.IsBigInt(X)&&xQ(Z)||O.IsString(X)&&v5(Z)||O.IsSymbol(X)&&h5(Z)||O.IsNumber(X)&&J6(Z)||O.IsInteger(X)&&J6(Z)||O.IsBoolean(X)&&y5(Z)||O.IsUint8Array(X)&&vQ(Z)||O.IsDate(X)&&gQ(Z)||O.IsConstructor(X)&&yQ(Z)||O.IsFunction(X)&&hQ(Z)?S.True:O.IsRecord(X)&&O.IsString(Y6(X))?(()=>{return Z[v0]==="Record"?S.True:S.False})():O.IsRecord(X)&&O.IsNumber(Y6(X))?(()=>{return E0(Z,0)?S.True:S.False})():S.False}function mQ(X,Z){return V0(Z)?_0(X,Z):O.IsRecord(Z)?d0(X,Z):!O.IsObject(Z)?S.False:(()=>{for(let $ of Object.getOwnPropertyNames(Z.properties)){if(!($ in X.properties)&&!O.IsOptional(Z.properties[$]))return S.False;if(O.IsOptional(Z.properties[$]))return S.True;if(l5(X.properties[$],Z.properties[$])===S.False)return S.False}return S.True})()}function dQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)&&uQ(Z)?S.True:!O.IsPromise(Z)?S.False:m0(o(X.item,Z.item))}function Y6(X){return D8 in X.patternProperties?h0():(L8 in X.patternProperties)?t0():$X("Unknown record key pattern")}function V6(X){return D8 in X.patternProperties?X.patternProperties[D8]:(L8 in X.patternProperties)?X.patternProperties[L8]:$X("Unable to get record value schema")}function d0(X,Z){let[$,H]=[Y6(Z),V6(Z)];return O.IsLiteralString(X)&&O.IsNumber($)&&m0(o(X,H))===S.True?S.True:O.IsUint8Array(X)&&O.IsNumber($)?o(X,H):O.IsString(X)&&O.IsNumber($)?o(X,H):O.IsArray(X)&&O.IsNumber($)?o(X,H):O.IsObject(X)?(()=>{for(let U of Object.getOwnPropertyNames(X.properties))if(l5(H,X.properties[U])===S.False)return S.False;return S.True})():S.False}function cQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):!O.IsRecord(Z)?S.False:o(V6(X),V6(Z))}function pQ(X,Z){let $=O.IsRegExp(X)?t0():X,H=O.IsRegExp(Z)?t0():Z;return o($,H)}function n5(X,Z){return O.IsLiteral(X)&&D0.IsString(X.const)?S.True:O.IsString(X)?S.True:S.False}function lQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):O.IsRecord(Z)?d0(X,Z):O.IsString(Z)?S.True:S.False}function nQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):O.IsRecord(Z)?d0(X,Z):O.IsSymbol(Z)?S.True:S.False}function iQ(X,Z){return O.IsTemplateLiteral(X)?o(g8(X),Z):O.IsTemplateLiteral(Z)?o(X,g8(Z)):$X("Invalid fallthrough for TemplateLiteral")}function oQ(X,Z){return O.IsArray(Z)&&X.items!==void 0&&X.items.every(($)=>o($,Z.items)===S.True)}function aQ(X,Z){return O.IsNever(X)?S.True:O.IsUnknown(X)?S.False:O.IsAny(X)?S.Union:S.False}function sQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)&&O2(Z)?S.True:O.IsArray(Z)&&oQ(X,Z)?S.True:!O.IsTuple(Z)?S.False:D0.IsUndefined(X.items)&&!D0.IsUndefined(Z.items)||!D0.IsUndefined(X.items)&&D0.IsUndefined(Z.items)?S.False:D0.IsUndefined(X.items)&&!D0.IsUndefined(Z.items)?S.True:X.items.every(($,H)=>o($,Z.items[H])===S.True)?S.True:S.False}function rQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):O.IsRecord(Z)?d0(X,Z):O.IsUint8Array(Z)?S.True:S.False}function tQ(X,Z){return V0(Z)?_0(X,Z):O.IsObject(Z)?F0(X,Z):O.IsRecord(Z)?d0(X,Z):O.IsVoid(Z)?Z1(X,Z):O.IsUndefined(Z)?S.True:S.False}function q6(X,Z){return Z.anyOf.some(($)=>o(X,$)===S.True)?S.True:S.False}function eQ(X,Z){return X.anyOf.every(($)=>o($,Z)===S.True)?S.True:S.False}function i5(X,Z){return S.True}function X1(X,Z){return O.IsNever(Z)?c5(X,Z):O.IsIntersect(Z)?L2(X,Z):O.IsUnion(Z)?q6(X,Z):O.IsAny(Z)?_6(X,Z):O.IsString(Z)?n5(X,Z):O.IsNumber(Z)?p5(X,Z):O.IsInteger(Z)?d5(X,Z):O.IsBoolean(Z)?m5(X,Z):O.IsArray(Z)?OQ(X,Z):O.IsTuple(Z)?aQ(X,Z):O.IsObject(Z)?F0(X,Z):O.IsUnknown(Z)?S.True:S.False}function Z1(X,Z){return O.IsUndefined(X)?S.True:O.IsUndefined(X)?S.True:S.False}function $1(X,Z){return O.IsIntersect(Z)?L2(X,Z):O.IsUnion(Z)?q6(X,Z):O.IsUnknown(Z)?i5(X,Z):O.IsAny(Z)?_6(X,Z):O.IsObject(Z)?F0(X,Z):O.IsVoid(Z)?S.True:S.False}function o(X,Z){return O.IsTemplateLiteral(X)||O.IsTemplateLiteral(Z)?iQ(X,Z):O.IsRegExp(X)||O.IsRegExp(Z)?pQ(X,Z):O.IsNot(X)||O.IsNot(Z)?kQ(X,Z):O.IsAny(X)?LQ(X,Z):O.IsArray(X)?FQ(X,Z):O.IsBigInt(X)?RQ(X,Z):O.IsBoolean(X)?KQ(X,Z):O.IsAsyncIterator(X)?NQ(X,Z):O.IsConstructor(X)?wQ(X,Z):O.IsDate(X)?SQ(X,Z):O.IsFunction(X)?jQ(X,Z):O.IsInteger(X)?CQ(X,Z):O.IsIntersect(X)?EQ(X,Z):O.IsIterator(X)?IQ(X,Z):O.IsLiteral(X)?TQ(X,Z):O.IsNever(X)?PQ(X,Z):O.IsNull(X)?bQ(X,Z):O.IsNumber(X)?fQ(X,Z):O.IsObject(X)?mQ(X,Z):O.IsRecord(X)?cQ(X,Z):O.IsString(X)?lQ(X,Z):O.IsSymbol(X)?nQ(X,Z):O.IsTuple(X)?sQ(X,Z):O.IsPromise(X)?dQ(X,Z):O.IsUint8Array(X)?rQ(X,Z):O.IsUndefined(X)?tQ(X,Z):O.IsUnion(X)?eQ(X,Z):O.IsUnknown(X)?X1(X,Z):O.IsVoid(X)?$1(X,Z):$X(\`Unknown left type operand '\${X[C]}'\`)}function R8(X,Z){return o(X,Z)}function z1(X,Z,$,H,U){let G={};for(let _ of globalThis.Object.getOwnPropertyNames(X))G[_]=zX(X[_],Z,$,H,t(U));return G}function Q1(X,Z,$,H,U){return z1(X.properties,Z,$,H,U)}function o5(X,Z,$,H,U){let G=Q1(X,Z,$,H,U);return y(G)}function W1(X,Z,$,H){let U=R8(X,Z);return U===S.Union?d([$,H]):U===S.True?$:H}function zX(X,Z,$,H,U){return e(X)?o5(X,Z,$,H,U):k0(X)?w(a5(X,Z,$,H,U)):w(W1(X,Z,$,H),U)}function H1(X,Z,$,H,U){return{[X]:zX(m(X),Z,$,H,t(U))}}function J1(X,Z,$,H,U){return X.reduce((G,_)=>{return{...G,...H1(_,Z,$,H,U)}},{})}function Y1(X,Z,$,H,U){return J1(X.keys,Z,$,H,U)}function a5(X,Z,$,H,U){let G=Y1(X,Z,$,H,U);return y(G)}function s5(X,Z){return QX(g8(X),Z)}function V1(X,Z){let $=X.filter((H)=>R8(H,Z)===S.False);return $.length===1?$[0]:d($)}function QX(X,Z,$={}){if(b0(X))return w(s5(X,Z),$);if(e(X))return w(r5(X,Z),$);return w(v(X)?V1(X.anyOf,Z):R8(X,Z)!==S.False?h():X,$)}function _1(X,Z){let $={};for(let H of globalThis.Object.getOwnPropertyNames(X))$[H]=QX(X[H],Z);return $}function q1(X,Z){return _1(X.properties,Z)}function r5(X,Z){let $=q1(X,Z);return y($)}function t5(X,Z){return WX(g8(X),Z)}function B1(X,Z){let $=X.filter((H)=>R8(H,Z)!==S.False);return $.length===1?$[0]:d($)}function WX(X,Z,$){if(b0(X))return w(t5(X,Z),$);if(e(X))return w(e5(X,Z),$);return w(v(X)?B1(X.anyOf,Z):R8(X,Z)!==S.False?X:h(),$)}function U1(X,Z){let $={};for(let H of globalThis.Object.getOwnPropertyNames(X))$[H]=WX(X[H],Z);return $}function G1(X,Z){return U1(X.properties,Z)}function e5(X,Z){let $=G1(X,Z);return y($)}function X4(X,Z){return o0(X)?w(X.returns,Z):h(Z)}function F2(X){return C0(L0(X))}function v8(X,Z,$){return w({[C]:"Record",type:"object",patternProperties:{[X]:Z}},$)}function B6(X,Z,$){let H={};for(let U of X)H[U]=Z;return i(H,{...$,[v0]:"Record"})}function M1(X,Z,$){return U5(X)?B6(j0(X),Z,$):v8(X.pattern,Z,$)}function A1(X,Z,$){return B6(j0(d(X)),Z,$)}function D1(X,Z,$){return B6([X.toString()],Z,$)}function L1(X,Z,$){return v8(X.source,Z,$)}function O1(X,Z,$){let H=H0(X.pattern)?L8:X.pattern;return v8(H,Z,$)}function F1(X,Z,$){return v8(L8,Z,$)}function N1(X,Z,$){return v8(Q5,Z,$)}function R1(X,Z,$){return i({true:Z,false:Z},$)}function K1(X,Z,$){return v8(D8,Z,$)}function w1(X,Z,$){return v8(D8,Z,$)}function N2(X,Z,$={}){return v(X)?A1(X.anyOf,Z,$):b0(X)?M1(X,Z,$):P0(X)?D1(X.const,Z,$):J8(X)?R1(X,Z,$):s0(X)?K1(X,Z,$):r0(X)?w1(X,Z,$):r2(X)?L1(X,Z,$):Y8(X)?O1(X,Z,$):a2(X)?F1(X,Z,$):M8(X)?N1(X,Z,$):h($)}function R2(X){return globalThis.Object.getOwnPropertyNames(X.patternProperties)[0]}function Z4(X){let Z=R2(X);return Z===L8?t0():Z===D8?h0():t0({pattern:Z})}function K2(X){return X.patternProperties[R2(X)]}function S1(X,Z){return Z.parameters=gX(X,Z.parameters),Z.returns=e0(X,Z.returns),Z}function j1(X,Z){return Z.parameters=gX(X,Z.parameters),Z.returns=e0(X,Z.returns),Z}function C1(X,Z){return Z.allOf=gX(X,Z.allOf),Z}function E1(X,Z){return Z.anyOf=gX(X,Z.anyOf),Z}function I1(X,Z){if(H0(Z.items))return Z;return Z.items=gX(X,Z.items),Z}function T1(X,Z){return Z.items=e0(X,Z.items),Z}function P1(X,Z){return Z.items=e0(X,Z.items),Z}function k1(X,Z){return Z.items=e0(X,Z.items),Z}function b1(X,Z){return Z.item=e0(X,Z.item),Z}function f1(X,Z){let $=h1(X,Z.properties);return{...Z,...i($)}}function x1(X,Z){let $=e0(X,Z4(Z)),H=e0(X,K2(Z)),U=N2($,H);return{...Z,...U}}function g1(X,Z){return Z.index in X?X[Z.index]:N8()}function v1(X,Z){let $=n8(Z),H=S0(Z),U=e0(X,Z);return $&&H?F2(U):$&&!H?C0(U):!$&&H?L0(U):U}function h1(X,Z){return globalThis.Object.getOwnPropertyNames(Z).reduce(($,H)=>{return{...$,[H]:v1(X,Z[H])}},{})}function gX(X,Z){return Z.map(($)=>e0(X,$))}function e0(X,Z){return o0(Z)?S1(X,Z):a0(Z)?j1(X,Z):Z0(Z)?C1(X,Z):v(Z)?E1(X,Z):f0(Z)?I1(X,Z):n0(Z)?T1(X,Z):T8(Z)?P1(X,Z):k8(Z)?k1(X,Z):b8(Z)?b1(X,Z):J0(Z)?f1(X,Z):f8(Z)?x1(X,Z):s2(Z)?g1(X,Z):Z}function $4(X,Z){return e0(Z,l8(X))}function z4(X){return w({[C]:"Integer",type:"integer"},X)}function y1(X,Z,$){return{[X]:X8(m(X),Z,t($))}}function u1(X,Z,$){return X.reduce((U,G)=>{return{...U,...y1(G,Z,$)}},{})}function m1(X,Z,$){return u1(X.keys,Z,$)}function Q4(X,Z,$){let H=m1(X,Z,$);return y(H)}function d1(X){let[Z,$]=[X.slice(0,1),X.slice(1)];return[Z.toLowerCase(),$].join("")}function c1(X){let[Z,$]=[X.slice(0,1),X.slice(1)];return[Z.toUpperCase(),$].join("")}function p1(X){return X.toUpperCase()}function l1(X){return X.toLowerCase()}function n1(X,Z,$){let H=r8(X.pattern);if(!x8(H))return{...X,pattern:W4(X.pattern,Z)};let _=[...kX(H)].map((W)=>m(W)),q=H4(_,Z),B=d(q);return V2([B],$)}function W4(X,Z){return typeof X==="string"?Z==="Uncapitalize"?d1(X):Z==="Capitalize"?c1(X):Z==="Uppercase"?p1(X):Z==="Lowercase"?l1(X):X:X.toString()}function H4(X,Z){return X.map(($)=>X8($,Z))}function X8(X,Z,$={}){return k0(X)?Q4(X,Z,$):b0(X)?n1(X,Z,$):v(X)?d(H4(X.anyOf,Z),$):P0(X)?m(W4(X.const,Z),$):w(X,$)}function J4(X,Z={}){return X8(X,"Capitalize",Z)}function Y4(X,Z={}){return X8(X,"Lowercase",Z)}function V4(X,Z={}){return X8(X,"Uncapitalize",Z)}function _4(X,Z={}){return X8(X,"Uppercase",Z)}function i1(X,Z,$){let H={};for(let U of globalThis.Object.getOwnPropertyNames(X))H[U]=K8(X[U],Z,t($));return H}function o1(X,Z,$){return i1(X.properties,Z,$)}function q4(X,Z,$){let H=o1(X,Z,$);return y(H)}function a1(X,Z){return X.map(($)=>U6($,Z))}function s1(X,Z){return X.map(($)=>U6($,Z))}function r1(X,Z){let{[Z]:$,...H}=X;return H}function t1(X,Z){return Z.reduce(($,H)=>r1($,H),X)}function e1(X,Z,$){let H=$0(X,[Y0,"$id","required","properties"]),U=t1($,Z);return i(U,H)}function XW(X){let Z=X.reduce(($,H)=>Q2(H)?[...$,m(H)]:$,[]);return d(Z)}function U6(X,Z){return Z0(X)?O0(a1(X.allOf,Z)):v(X)?d(s1(X.anyOf,Z)):J0(X)?e1(X,Z,X.properties):i({})}function K8(X,Z,$){let H=W0(Z)?XW(Z):Z,U=x0(Z)?j0(Z):Z,G=Q0(X),_=Q0(Z);return e(X)?q4(X,U,$):k0(Z)?B4(X,Z,$):G&&_?r("Omit",[X,H],$):!G&&_?r("Omit",[X,H],$):G&&!_?r("Omit",[X,H],$):w({...U6(X,U),...$})}function ZW(X,Z,$){return{[Z]:K8(X,[Z],t($))}}function $W(X,Z,$){return Z.reduce((H,U)=>{return{...H,...ZW(X,U,$)}},{})}function zW(X,Z,$){return $W(X,Z.keys,$)}function B4(X,Z,$){let H=zW(X,Z,$);return y(H)}function QW(X,Z,$){let H={};for(let U of globalThis.Object.getOwnPropertyNames(X))H[U]=w8(X[U],Z,t($));return H}function WW(X,Z,$){return QW(X.properties,Z,$)}function U4(X,Z,$){let H=WW(X,Z,$);return y(H)}function HW(X,Z){return X.map(($)=>G6($,Z))}function JW(X,Z){return X.map(($)=>G6($,Z))}function YW(X,Z){let $={};for(let H of Z)if(H in X)$[H]=X[H];return $}function VW(X,Z,$){let H=$0(X,[Y0,"$id","required","properties"]),U=YW($,Z);return i(U,H)}function _W(X){let Z=X.reduce(($,H)=>Q2(H)?[...$,m(H)]:$,[]);return d(Z)}function G6(X,Z){return Z0(X)?O0(HW(X.allOf,Z)):v(X)?d(JW(X.anyOf,Z)):J0(X)?VW(X,Z,X.properties):i({})}function w8(X,Z,$){let H=W0(Z)?_W(Z):Z,U=x0(Z)?j0(Z):Z,G=Q0(X),_=Q0(Z);return e(X)?U4(X,U,$):k0(Z)?G4(X,Z,$):G&&_?r("Pick",[X,H],$):!G&&_?r("Pick",[X,H],$):G&&!_?r("Pick",[X,H],$):w({...G6(X,U),...$})}function qW(X,Z,$){return{[Z]:w8(X,[Z],t($))}}function BW(X,Z,$){return Z.reduce((H,U)=>{return{...H,...qW(X,U,$)}},{})}function UW(X,Z,$){return BW(X,Z.keys,$)}function G4(X,Z,$){let H=UW(X,Z,$);return y(H)}function GW(X,Z){return r("Partial",[r(X,Z)])}function MW(X){return r("Partial",[z8(X)])}function AW(X){let Z={};for(let $ of globalThis.Object.getOwnPropertyNames(X))Z[$]=L0(X[$]);return Z}function DW(X,Z){let $=$0(X,[Y0,"$id","required","properties"]),H=AW(Z);return i(H,$)}function M4(X){return X.map((Z)=>A4(Z))}function A4(X){return i0(X)?GW(X.target,X.parameters):Q0(X)?MW(X.$ref):Z0(X)?O0(M4(X.allOf)):v(X)?d(M4(X.anyOf)):J0(X)?DW(X,X.properties):P8(X)?X:J8(X)?X:s0(X)?X:P0(X)?X:SX(X)?X:r0(X)?X:Y8(X)?X:jX(X)?X:CX(X)?X:i({})}function HX(X,Z){if(e(X))return D4(X,Z);else return w({...A4(X),...Z})}function LW(X,Z){let $={};for(let H of globalThis.Object.getOwnPropertyNames(X))$[H]=HX(X[H],t(Z));return $}function OW(X,Z){return LW(X.properties,Z)}function D4(X,Z){let $=OW(X,Z);return y($)}function FW(X,Z){return r("Required",[r(X,Z)])}function NW(X){return r("Required",[z8(X)])}function RW(X){let Z={};for(let $ of globalThis.Object.getOwnPropertyNames(X))Z[$]=$0(X[$],[A0]);return Z}function KW(X,Z){let $=$0(X,[Y0,"$id","required","properties"]),H=RW(Z);return i(H,$)}function L4(X){return X.map((Z)=>O4(Z))}function O4(X){return i0(X)?FW(X.target,X.parameters):Q0(X)?NW(X.$ref):Z0(X)?O0(L4(X.allOf)):v(X)?d(L4(X.anyOf)):J0(X)?KW(X,X.properties):P8(X)?X:J8(X)?X:s0(X)?X:P0(X)?X:SX(X)?X:r0(X)?X:Y8(X)?X:jX(X)?X:CX(X)?X:i({})}function JX(X,Z){if(e(X))return F4(X,Z);else return w({...O4(X),...Z})}function wW(X,Z){let $={};for(let H of globalThis.Object.getOwnPropertyNames(X))$[H]=JX(X[H],Z);return $}function SW(X,Z){return wW(X.properties,Z)}function F4(X,Z){let $=SW(X,Z);return y($)}function jW(X,Z){return Z.map(($)=>{return Q0($)?M6(X,$.$ref):g0(X,$)})}function M6(X,Z){return Z in X?Q0(X[Z])?M6(X,X[Z].$ref):g0(X,X[Z]):h()}function CW(X){return XX(X[0])}function EW(X){return F8(X[0],X[1])}function IW(X){return ZX(X[0])}function TW(X){return HX(X[0])}function PW(X){return K8(X[0],X[1])}function kW(X){return w8(X[0],X[1])}function bW(X){return JX(X[0])}function fW(X,Z,$){let H=jW(X,$);return Z==="Awaited"?CW(H):Z==="Index"?EW(H):Z==="KeyOf"?IW(H):Z==="Partial"?TW(H):Z==="Omit"?PW(H):Z==="Pick"?kW(H):Z==="Required"?bW(H):h()}function xW(X,Z){return i8(g0(X,Z))}function gW(X,Z){return o8(g0(X,Z))}function vW(X,Z,$){return a8(vX(X,Z),g0(X,$))}function hW(X,Z,$){return Z8(vX(X,Z),g0(X,$))}function yW(X,Z){return O0(vX(X,Z))}function uW(X,Z){return e8(g0(X,Z))}function mW(X,Z){return i(globalThis.Object.keys(Z).reduce(($,H)=>{return{...$,[H]:g0(X,Z[H])}},{}))}function dW(X,Z){let[$,H]=[g0(X,K2(Z)),R2(Z)],U=l8(Z);return U.patternProperties[H]=$,U}function cW(X,Z){return Q0(Z)?{...M6(X,Z.$ref),[Y0]:Z[Y0]}:Z}function pW(X,Z){return y0(vX(X,Z))}function lW(X,Z){return d(vX(X,Z))}function vX(X,Z){return Z.map(($)=>g0(X,$))}function g0(X,Z){return S0(Z)?w(g0(X,$0(Z,[A0])),Z):n8(Z)?w(g0(X,$0(Z,[l0])),Z):A8(Z)?w(cW(X,Z),Z):n0(Z)?w(xW(X,Z.items),Z):T8(Z)?w(gW(X,Z.items),Z):i0(Z)?w(fW(X,Z.target,Z.parameters)):o0(Z)?w(vW(X,Z.parameters,Z.returns),Z):a0(Z)?w(hW(X,Z.parameters,Z.returns),Z):Z0(Z)?w(yW(X,Z.allOf),Z):k8(Z)?w(uW(X,Z.items),Z):J0(Z)?w(mW(X,Z.properties),Z):f8(Z)?w(dW(X,Z)):f0(Z)?w(pW(X,Z.items||[]),Z):v(Z)?w(lW(X,Z.anyOf),Z):Z}function nW(X,Z){return Z in X?g0(X,X[Z]):h()}function N4(X){return globalThis.Object.getOwnPropertyNames(X).reduce((Z,$)=>{return{...Z,[$]:nW(X,$)}},{})}class R4{constructor(X){let Z=N4(X),$=this.WithIdentifiers(Z);this.$defs=$}Import(X,Z){let $={...this.$defs,[X]:w(this.$defs[X],Z)};return w({[C]:"Import",$defs:$,$ref:X})}WithIdentifiers(X){return globalThis.Object.getOwnPropertyNames(X).reduce((Z,$)=>{return{...Z,[$]:{...X[$],$id:$}}},{})}}function K4(X){return new R4(X)}function w4(X,Z){return w({[C]:"Not",not:X},Z)}function S4(X,Z){return a0(X)?y0(X.parameters,Z):h()}var iW=0;function j4(X,Z={}){if(H0(Z.$id))Z.$id=\`T\${iW++}\`;let $=l8(X({[C]:"This",$ref:\`\${Z.$id}\`}));return $.$id=Z.$id,w({[v0]:"Recursive",...$},Z)}function C4(X,Z){let $=a(X)?new globalThis.RegExp(X):X;return w({[C]:"RegExp",type:"RegExp",source:$.source,flags:$.flags},Z)}function oW(X){return Z0(X)?X.allOf:v(X)?X.anyOf:f0(X)?X.items??[]:[]}function E4(X){return oW(X)}function I4(X,Z){return a0(X)?w(X.returns,Z):h(Z)}class T4{constructor(X){this.schema=X}Decode(X){return new P4(this.schema,X)}}class P4{constructor(X,Z){this.schema=X,this.decode=Z}EncodeTransform(X,Z){let U={Encode:(G)=>Z[Y0].Encode(X(G)),Decode:(G)=>this.decode(Z[Y0].Decode(G))};return{...Z,[Y0]:U}}EncodeSchema(X,Z){let $={Decode:this.decode,Encode:X};return{...Z,[Y0]:$}}Encode(X){return A8(this.schema)?this.EncodeTransform(X,this.schema):this.EncodeSchema(X,this.schema)}}function k4(X){return new T4(X)}function b4(X={}){return w({[C]:X[C]??"Unsafe"},X)}function f4(X){return w({[C]:"Void",type:"void"},X)}var A6={};C2(A6,{Void:()=>f4,Uppercase:()=>_4,Unsafe:()=>b4,Unknown:()=>N8,Union:()=>d,Undefined:()=>M2,Uncapitalize:()=>V4,Uint8Array:()=>A2,Tuple:()=>y0,Transform:()=>k4,TemplateLiteral:()=>V2,Symbol:()=>G2,String:()=>t0,ReturnType:()=>I4,Rest:()=>E4,Required:()=>JX,RegExp:()=>C4,Ref:()=>z8,Recursive:()=>j4,Record:()=>N2,ReadonlyOptional:()=>F2,Readonly:()=>C0,Promise:()=>q2,Pick:()=>w8,Partial:()=>HX,Parameters:()=>S4,Optional:()=>L0,Omit:()=>K8,Object:()=>i,Number:()=>h0,Null:()=>U2,Not:()=>w4,Never:()=>h,Module:()=>K4,Mapped:()=>j5,Lowercase:()=>Y4,Literal:()=>m,KeyOf:()=>ZX,Iterator:()=>e8,Intersect:()=>O0,Integer:()=>z4,Instantiate:()=>$4,InstanceType:()=>X4,Index:()=>F8,Function:()=>Z8,Extract:()=>WX,Extends:()=>zX,Exclude:()=>QX,Enum:()=>x5,Date:()=>B2,ConstructorParameters:()=>f5,Constructor:()=>a8,Const:()=>b5,Composite:()=>k5,Capitalize:()=>J4,Boolean:()=>Y2,BigInt:()=>t8,Awaited:()=>XX,AsyncIterator:()=>o8,Array:()=>i8,Argument:()=>V5,Any:()=>O8});var N0=A6;var x4="http://127.0.0.1:3850",YX="pi",h8="plugin",q8=5000,VX=1e4,g4=5000,w2="signet-pi-hidden-recall",S2="signet-pi-session-context";function v4(X){return u2(X,{logPrefix:"signet-pi",actorName:"pi-extension",runtimePath:h8,defaultTimeout:q8})}var aW=new Set([w2,S2]),h4={harness:YX,runtimePath:h8,writeTimeout:VX,promptSubmitTimeout:g4,excludedCustomTypes:aW,sessionStartTimeout:()=>q8,staticFallback:m2};function y4(X,Z){return{role:"custom",customType:X,display:!1,content:\`<signet-memory source="auto-recall">
\${c2(Z)}
</signet-memory>\`,timestamp:Date.now()}}class u4 extends X2{consumeHiddenInjectMessages(X){if(!X)return[];let Z=[],$=c(this.pendingSessionContext.get(X));if($)Z.push(y4(S2,$));this.pendingSessionContext.delete(X);let H=this.consumePendingRecall(X);if(H)Z.push(y4(w2,H));return Z}}function m4(){return new u4}function eW(){let X=tW(ZZ(),"extensions","signet.json");if(!sW(X))return null;try{let Z=rW(X,"utf-8"),$=JSON.parse(Z);if(typeof $!=="object"||$===null)return null;return $}catch{return null}}function XH(){let X=eW(),Z=LX("SIGNET_ENABLED"),$=X?.enabled;return{enabled:Z!==void 0?Z!=="false":$!==void 0?$:!0}}var ZH=XH(),_X={lastRecall:null,memoryCount:0};async function y8(X){try{return(await fetch(\`\${X}/health\`,{method:"GET",headers:{Accept:"application/json"},signal:AbortSignal.timeout(q8)})).ok}catch{return!1}}async function d4(X,Z,$={}){let{limit:H=10,agentId:U,scope:G}=$,_=await fetch(\`\${X}/api/memory/recall\`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({query:Z,limit:H,agentId:U,...G!==void 0&&{scope:G}}),signal:AbortSignal.timeout(q8)});if(!_.ok){let B=await _.text();throw Error(\`Recall failed: \${B}\`)}return((await _.json()).results??[]).map((B)=>({id:B.id,content:B.content,importance:B.importance,tags:typeof B.tags==="string"?B.tags.split(",").map((W)=>W.trim()).filter(Boolean):void 0}))}async function c4(X,Z,$={}){let{critical:H=!1,tags:U=[],agentId:G}=$,_=await fetch(\`\${X}/api/hooks/remember\`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({harness:YX,content:Z,pinned:H,tags:U,agentId:G,source:"pi-extension"}),signal:AbortSignal.timeout(VX)});if(!_.ok){let q=await _.text();throw Error(\`Remember failed: \${q}\`)}}async function $H(X,Z,$,H={}){let U=await fetch(\`\${X}/api/memory/feedback\`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sessionKey:Z,agentId:H.agentId??"default",feedback:$}),signal:AbortSignal.timeout(VX)});if(!U.ok){let G=await U.text();throw Error(\`Feedback failed: \${G}\`)}return await U.json()}function D6(X){let Z=_X.lastRecall?\`signet:\${_X.memoryCount} memories\`:"signet:ready";X.ui.setStatus("signet",X.ui.theme.fg("accent",Z))}function zH(X,Z,$){X.on("session_start",async(H,U)=>{if(await y8($))U.ui.notify("SignetAI memory connected","info"),D6(U);else U.ui.notify("SignetAI daemon not running. Memory features disabled.","warning"),U.ui.notify("Install: npm install -g signetai && signet setup","info");await G8(Z,U)}),X.on("session_switch",async(H,U)=>{await FX(Z,H,H.type),await G8(Z,U)}),X.on("session_fork",async(H,U)=>{await FX(Z,H,H.type),await G8(Z,U)}),X.on("session_shutdown",async(H,U)=>{U.ui.setStatus("signet",void 0),await tX(Z,U,"session_shutdown")})}function QH(X,Z){X.on("input",async($,H)=>{let U=I0(H);Z.state.clearPendingRecall(U.sessionId),await NX(Z,H,$.text)}),X.on("before_agent_start",async($,H)=>{await p8(Z,H);let U=I0(H);if(!U.sessionId)return;if(Z.state.hasPendingRecall(U.sessionId))return;await NX(Z,H,$.prompt)})}function WH(X,Z){X.on("context",async($,H)=>{let U=I0(H),G=Z.state.consumeHiddenInjectMessages(U.sessionId);if(G.length===0)return;return{messages:[...$.messages,...G]}})}function HH(X,Z){X.on("session_before_compact",async($,H)=>{await p8(Z,H);let U=I0(H);await Z.client.post("/api/hooks/pre-compaction",{harness:YX,sessionKey:U.sessionId,messageCount:Array.isArray($.preparation?.messagesToSummarize)?$.preparation.messagesToSummarize.length:void 0,runtimePath:h8},q8);return}),X.on("session_compact",async($,H)=>{let U=c($.compactionEntry?.summary);if(!U)return;let G=I0(H);await Z.client.post("/api/hooks/compaction-complete",{harness:YX,summary:U,project:G.project,sessionKey:G.sessionId,agentId:Z.agentId,runtimePath:h8},VX)})}function JH(X){let Z=X.trim(),$=!1,H=[];if(Z.startsWith("critical:"))$=!0,Z=Z.slice(9).trim();let U=Z.match(/^\\[([^\\]]+)\\]:\\s*/);if(U)H.push(...U[1].split(",").map((G)=>G.trim())),Z=Z.slice(U[0].length);return{content:Z,critical:$,tags:H}}function YH(X,Z,$){X.registerCommand("recall",{description:"Search SignetAI memories",handler:async(H,U)=>{if(!H?.trim()){U.ui.notify("Usage: /recall <query>","warning");return}if(!await y8(Z)){U.ui.notify("Signet daemon not running. Run: signet daemon start","error");return}U.ui.notify(\`Recalling: "\${H}"...\`,"info");try{let _=await d4(Z,H,{limit:5,agentId:$});if(_.length===0){U.ui.notify("No relevant memories found","info");return}_X.lastRecall=new Date().toISOString(),_X.memoryCount=_.length,D6(U);let q=_.map((B,W)=>{let V=B.tags?.length?\`[\${B.tags.join(", ")}] \`:"";return\`\${W+1}. [\${B.id}] \${V}\${B.content}\`}).join(\`
\`);U.ui.notify(\`Found \${_.length} memories:
\${q}\`,"success")}catch(_){let q=_ instanceof Error?_.message:String(_);U.ui.notify(\`Recall failed: \${q}\`,"error")}}}),X.registerCommand("remember",{description:"Save a memory to SignetAI",handler:async(H,U)=>{if(!H?.trim()){U.ui.notify("Usage: /remember <content>","warning");return}if(!await y8(Z)){U.ui.notify("Signet daemon not running. Run: signet daemon start","error");return}let{content:_,critical:q,tags:B}=JH(H);try{await c4(Z,_,{critical:q,tags:B,agentId:$});let W=q?" (pinned)":"";U.ui.notify(\`Memory saved\${W}: "\${_.substring(0,50)}..."\`,"success")}catch(W){let V=W instanceof Error?W.message:String(W);U.ui.notify(\`Remember failed: \${V}\`,"error")}}}),X.registerCommand("signet-status",{description:"Check SignetAI daemon status",handler:async(H,U)=>{let G=await y8(Z),_=U.sessionManager.getSessionId();if(G){let q=[\`Signet daemon is running on \${Z}\`];if(_)q.push(\`Session: \${_}\`);U.ui.notify(q.join(\`
