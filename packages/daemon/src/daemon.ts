...
async function syncHarnessConfigs() {
	...
	const buildHeader = (targetName: string) => {
		...
		const fileList = existingFiles.map((f) => `#   - ${join(AGENTS_DIR, f.name)} (${f.desc})`).join("\n");

		return `# ${targetName}
# ============================================================================
# AUTO-GENERATED from ${agentsMdPath} by Signet
# Generated: ${new Date().toISOString()}
# 
# DO NOT EDIT THIS FILE - changes will be overwritten
# Edit the source file instead: ${agentsMdPath}
#
# Signet Agent Home: ${AGENTS_DIR}
